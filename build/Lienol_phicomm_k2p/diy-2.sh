...
# 内核显示增加自己个性名称（4444 build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些为后期增加）
#sed -i "s/OpenWrt /4444 build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" package/default-settings/files/zzz-default-settings

# 修改 argon 为默认主题,可根据你喜欢的修改成其他的（不选择那些会自动改变为默认主题的主题才有效果）
sed -i 's/luci-theme-bootstrap/luci-theme-argon/g' feeds/luci/collections/luci/Makefile

# 修改默认wifi名称ssid为Xiaomi_R4A
sed -i 's/ssid=OpenWrt/ssid=@舒仪/g' package/kernel/mac80211/files/lib/wifi/mac80211.sh
